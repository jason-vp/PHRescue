...
<div id='resultados' class='borderUp'>
    ...
    <table>
        ...
        <?php for($i=0; $i<5; $i++): ?>
            <tr>
                <td>GAR</td>
                <td>Germany Animal Rescue</td>
                <td>Alemania</td>
                <td><button type='submit' title='Ver Detalles' class='buEdit' onclick="openMultiOverlay(1, 'asociacion')"></button><button type='submit' title='Eliminar' class='buDelete'></button></td>
            </tr>
        <?php endfor; ?>
...
@section ('autoScripts')
    <?php
    if($adoptante && $type) {
        echo "<script type='text/javascript'> openMultiOverlay(\"" . $type . "\", " . $adoptante . ");</script>";
    }
            echo $adoptante." ".$type;
    ?>
@stop